...
| `--sync-manifest` | Сверяет размеры в `models.json` с Hugging Face |
| `--sync-manifest --write` | То же самое, но вписывает новые размеры |
...
Всего 44 проверки, за четырьмя из них 2640 порождённых, весь прогон - пять секунд. Проверками накрыты и консоль (коды возврата, разбор команды, установка от начала до конца), и окно (сборка виджетов, насос событий, три исхода закачки), и сама сборка, и `setup.nsi`.
...
Проверяется только размер, не контрольная сумма. Тут раньше было написано, что Hugging Face не отдаёт sha256 обычным запросом, - это неправда: отдаёт, в описи репозитория, той самой, по которой работает `--sync-manifest`. Верна вторая половина: считать хеш от 95 ГиБ на каждой проверке долго, и `--check` остаётся по размеру. Совпадение размера байт в байт отсекает всё, что реально ломается: оборванную закачку, пустой файл, не тот квант. Считать сумму на лету, пока байты и так текут через скачивание, можно почти даром - но это уже не сделано.
