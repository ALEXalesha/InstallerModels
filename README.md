...
|---|---|---|
| `InstallerModels-Setup-1.0.4.exe` | обычная установка с ярлыками | ничего |
...
Запусти `InstallerModels-Setup-1.0.4.exe`. Ставится в папку пользователя (`%LOCALAPPDATA%\Programs\InstallerModels`), права администратора не нужны, UAC не выскочит. Создаёт ярлык в меню Пуск, запись в «Программы и компоненты» для нормального удаления и, если не снять галочку, ярлык на рабочем столе. Галочка снимается на странице выбора компонентов.
