...
**Возвращает модели ComfyUI обратно на диск: качает с Hugging Face напрямую, с докачкой после обрыва, проверкой размера и sha256. 15 файлов на 95.4 ГиБ, разложенных по группам под конкретные воркфлоу. Свои модели добавляются ссылкой - размер, контрольную сумму и настройки программа спрашивает у Hugging Face сама.**

[Скачать для Windows](https://github.com/ALEXalesha/InstallerModels/releases/latest) &nbsp;·&nbsp; [English version of this file](README.md)
...
<img src="docs/screen-comfyui.png" width="820" alt="Окно InstallerModels">
...
|---|---|---|
| `InstallerModels-Setup-2.2.0.exe` | обычная установка с ярлыками | ничего |
| `InstallerModels-portable-2.2.0.zip` | распаковать и запускать, без установки | ничего |
| `python install.py` | из консоли, скриптом | Python 3.8+ |

Готовые файлы лежат в `dist/` после сборки, в git их нет. Как собрать - в [docs/build.md](docs/build.md).

С версии 2.0 окно сделано на Qt (PySide6), а не на tkinter. У Tk каждый виджет - отдельное окно Windows, и при изменении размера программа заметно тормозила; Qt рисует окно целиком и не тормозит. Заодно окно само следует светлой или тёмной теме Windows, а кнопки в диалогах подписаны по-русски.

### Установка

Запусти `InstallerModels-Setup-2.2.0.exe`. Ставится в папку пользователя (`%LOCALAPPDATA%\Programs\InstallerModels`), права администратора не нужны, UAC не выскочит. Создаёт ярлык в меню Пуск, запись в «Программы и компоненты» для нормального удаления и, если не снять галочку, ярлык на рабочем столе. Галочка снимается на странице выбора компонентов.
...
Распакуй `InstallerModels-portable-2.2.0.zip` в любую папку (можно на флешку) и запусти `InstallerModels\InstallerModels.exe`. Ничего никуда не пишет, кроме самих моделей, запомненной папки ComfyUI и места окна (с 2.2.0 окно открывается там и такого размера, где его закрыли). Рядом с exe лежит `models.json` - его можно править, программа возьмёт его вместо встроенного.

До версии 2.0 portable был одним exe. С Qt такой exe при каждом запуске распаковывал бы себя во временную папку - десятки мегабайт и секунды ожидания, - поэтому теперь это та же папка, что ставит установщик, только в архиве. Запускается она меньше чем за секунду.
...
Под списком групп - поле «Своя модель: ссылка на файл в Hugging Face»: вставь ссылку, нажми «Добавить», и файл появится в списке отдельной группой. Подробности в разделе «Своя модель по ссылке». Пока идёт закачка, кнопка заперта: очередь в потоке собрана по старому списку.

«Проверить файлы» пересматривает диск и обновляет статусы. Полезно, если ты что-то удалил вручную.

Вкладка LM Studio - названия моделей с кнопкой «Копировать» и таким же полем для своей модели внизу. Рядом с квантом показан ключ модели в LM Studio - тот, которым её зовут из `lms load` и из API. Он лежал в `models.json` и был расписан в документации, а сама программа его не показывала нигде. Скопированное переживает закрытие окна: Qt при выходе сам отдаёт буфер обмена системе. (На tkinter до версии 1.0.5 «скопировал и закрыл» означало «вставлять нечего» - ровно тот порядок действий, на который кнопка и рассчитана.)
...
| `--verify [ГРУППА ...]` | Пересчитывает sha256 у скачанного. Медленно: читает файлы целиком |
| `--remove ГРУППА ...` | Показывает, что удалилось бы, и сколько освободится |
| `--remove ГРУППА ... --yes` | Удаляет на самом деле |
...
| `--sync-manifest` | Сверяет размеры и суммы в `models.json` с Hugging Face |
| `--sync-manifest --write` | То же самое, но вписывает их в файл |
| `--add ССЫЛКА` | Добавляет свою модель в `models.json` по ссылке на Hugging Face |
| `--add ССЫЛКА --lmstudio` | То же для раздела LM Studio |
| `--add ССЫЛКА --dest ПУТЬ --group ИМЯ` | Куда положить и в какую группу, если догадка не подходит |
...
Если выбранный путь никуда не ведёт, программа сама ищет ComfyUI в обычных местах: в `Документах`, в домашней папке и в корнях дисков C-H, включая раскладку `ComfyUI_windows_portable\ComfyUI`. Папкой ComfyUI считается та, где есть `models` и рядом хоть один спутник - `custom_nodes`, `input`, `output`, `user`, `main.py` или `comfy`. Приметы сняты с настоящей установки: у ComfyUI Desktop нет ни `main.py`, ни папки `comfy`, так что искать по ним было бы бесполезно.

Диски целиком не обходятся - это минуты работы винта на ровном месте. Проверяется короткий список обычных мест, и только тогда, когда записанный путь и так оказался пустым местом.

**Названное прямо не подменяется никогда.** Если ты сам указал `--root` или `COMFYUI_ROOT`, программа возьмёт именно его, даже если такой папки нет, и скажет «папка не найдена». Опечатка в пути не должна молча уводить закачку на 95 ГиБ в другое место - об этом ты узнал бы только по кончившемуся диску.

Порядок такой: `--root`, потом `COMFYUI_ROOT`, потом папка, выбранная «Обзором», потом `models.json`, и лишь затем поиск. Один и тот же порядок у окна и у консоли. До версии 1.0.4 выбранную папку знало только окно, а `install.py` каждый раз начинал с пути из `models.json`: выберешь папку мышкой, запустишь `--check` в консоли - и она пересчитывает совсем другую папку, честно сообщая, что ничего не установлено.

## Перенос на другое устройство

Программа переносится копированием: возьми `dist/InstallerModels-portable-2.2.0.zip` (или папку `dist/app/InstallerModels` целиком) - там exe, `_internal`, `models.json`, оба README и `docs`. Python на новой машине не нужен, он внутри. Нужны только Windows и x64. Либо просто поставь там `InstallerModels-Setup`.

Что **не** переезжает - запомненная папка ComfyUI. Она лежит в `%LOCALAPPDATA%\InstallerModels\settings.json`, то есть привязана к машине и к учётной записи. На новой машине программа возьмёт путь из `models.json`, а он указывает в профиль того, кто этот файл правил. Если папка не найдётся, включится поиск по обычным местам, и чаще всего ComfyUI найдётся сам. Если нет - ткни «Обзор» один раз, дальше запомнится.

Если везёшь и сами модели, после копирования стоит прогнать:

```bash
python install.py --verify
```

Копирование 95 ГиБ по сети или через флешку - как раз тот случай, где файл может побиться, а размер при этом останется прежним. Обычная проверка такого не заметит, сверка контрольных сумм заметит.
...
| `gui.py` | Окно на Qt (PySide6) |
| `install.py` | Консольный вариант |
| `build.ps1` | Точка входа сборки: создаёт `.venv` проекта, ставит зависимости, зовёт `build.py` |
| `build.py` | Сборка папки программы, portable-архива и установщика |
| `requirements.txt`, `requirements-dev.txt` | PySide6 для окна; PyInstaller для сборки, hypothesis для свойств |
...
| `tests_props.py` | Свойства добавления по ссылке: гипотезы вместо примеров |
...
`tests.py` запускается сам по себе - `.venv\Scripts\python.exe tests.py` - и не требует сети; из сторонних библиотек ему нужен только PySide6 для проверок окна, и живёт он в `.venv` проекта, а не в общем Python. Hugging Face в нём изображает локальный сервер, которому можно велеть рвать соединение, отдавать не тот размер или молчать в ответ.

Проверок три пачки, и устроены они по-разному. В `tests.py` - список поломок, которые уже случались: каждая написана после того, как баг нашёлся, и названа так, чтобы из отчёта было понятно, что именно сломалось. Такие проверки по построению ловят баг только во второй раз.
...
В `tests_props.py` - третий способ: он не знает даже пространства. Он описывает, каким бывает вход - ссылка, опись репозитория, состояние манифеста, - а hypothesis сама ищет тот, на котором правило ломается, и ужимает найденное до кратчайшего примера. В первый же прогон так нашлись три настоящие ошибки: квант брался первым совпадением в имени, а не последним (`Qwen3-Q8-preview-Q4_K_M.gguf` записался бы как `Q8`); файл с именем устройства объяснялся жалобой на `models.json`, которого человек не трогал; а `https://example.com/a/b/...` разбиралось как репозиторий `example.com/a`.

Всего 89 проверок, за семью из них больше шести тысяч порождённых. Проверками накрыты и консоль (коды возврата, разбор команды, установка от начала до конца), и окно (сборка виджетов, насос событий, три исхода закачки, 300 случайных действий подряд с проверкой инвариантов после каждого, поиск окна по заголовку так же, как это делает установщик, скорость ресайза), и сама сборка, и `setup.nsi`.

Подробно про инварианты, оси перебора и мутационный прогон - в [docs/tests.md](docs/tests.md).

## Своя модель по ссылке (с версии 2.1)

В списке лежат модели под конкретные воркфлоу - чужому человеку нужны другие. Раньше для этого приходилось править `models.json` руками и самому добывать точный размер в байтах: ошибка на четыре байта останавливает скачивание сообщением «manifest is out of date», то есть чинить отправляли число, которое ты же и переписал.

Теперь достаточно ссылки. В окне, на вкладке ComfyUI под списком групп, есть поле «Своя модель: ссылка на файл в Hugging Face». В консоли:

```bash
python install.py --add "https://huggingface.co/автор/репозиторий/blob/main/файл.safetensors"
```

Программа спрашивает у Hugging Face опись репозитория - ту же, по которой работает `--sync-manifest`, - и берёт оттуда точный размер и sha256. Добавленная запись с первой секунды проверяется так же, как те пятнадцать, что лежали в манифесте изначально.

Ссылку понимают в любом виде, в каком её копируют: `/resolve/`, `/blob/`, `/raw/`, с хвостом `?download=true`, с процентными кодами, без `https://` и короткой записью `автор/репозиторий/путь`. Ветка только `main`: другую программа качать не умеет и потому честно отказывается, а не делает вид.

Папку внутри ComfyUI программа угадывает по тому, как файлы разложены в репозитории: `split_files/text_encoders/...` идёт в `models/text_encoders`, `.gguf` - в `models/unet` (в ComfyUI это квантованный unet для GGUF-нод, а не чекпойнт), файл со словом `lora` в имени - в `models/loras`. Догадка названа догадкой в выводе, и её можно не угадывать:

```bash
python install.py --add "ссылка" --dest models/loras/моя-lora.safetensors --group свои
```

Без `--group` файл ложится в группу `custom` («Добавленные вручную»), она создаётся при первом добавлении. `--dry-run` показывает, что добавилось бы, и ничего не записывает.

Отказ ничего не меняет: если репозитория нет, файл не найден, связь оборвалась или запись не годится, манифест остаётся ровно таким, каким был. Половина записи в нём хуже, чем её отсутствие - следующий запуск читает манифест целиком и остановится на кривой строке.

### LM Studio: то же самое, только настройки программа выясняет сама

На вкладке «LM Studio» внизу такое же поле, в консоли - `--add ССЫЛКА --lmstudio`. Тут известен только адрес репозитория, поэтому остальное программа выясняет:

- **файлы и их точные размеры с суммами** - из описи репозитория;
- **квант** - из имени выбранного файла; по умолчанию берётся `Q4_K_M`, а если нужен другой, дай ссылку прямо на нужный `.gguf`;
- **спутник `mmproj`** - по имени; без него модель перестанет принимать картинки;
- **ключ для `lms load`** - из `base_model` в карточке репозитория: `google/gemma-4-E2B-it` даёт `google/gemma-4-e2b`.

Что из этого выведено догадкой, а не прочитано с сервера, программа перечисляет прямо в отчёте. Ключ - именно догадка; он необязателен, и без него всё работает как раньше.

Подробности: [docs/models.md](docs/models.md) и [docs/lmstudio.md](docs/lmstudio.md).

## Если поменялся состав моделей

Руками править `models.json` по-прежнему можно, и программа по-прежнему ничего не угадывает - она читает этот файл. Чтобы добавить модель вручную, допиши в нужную группу запись:
...
`size` и `sha256` можно не выписывать руками - их проставит `--sync-manifest --write`. Поле `sha256` необязательное: без него всё работает как раньше, только целостность проверяется по одному размеру.

Размер можно не искать руками - его спросит сама программа:

```bash
python install.py --sync-manifest
```

Она обходит все репозитории из `models.json` - по одному запросу на репозиторий, а не на файл - и показывает, где размер разошёлся, а где путь вообще исчез. Скачивать при этом ничего не надо: опись репозитория отдаётся отдельной ручкой вместе с размерами.

```bash
python install.py --sync-manifest --write
```

вписывает новые размеры и контрольные суммы в `models.json`. Суммы сервер отдаёт в той же описи, за тот же запрос - считать их самому значило бы скачать все 95 ГиБ. Правятся только числа: путь, уехавший в никуда, программа не угадывает - какой файл автор имел в виду, знает только человек. Если до какого-то репозитория не достучались, не записывается ничего: записать половину и отчитаться «сверено» - худшее из возможного, человек решит, что манифест теперь верен целиком.

Проверено на деле: первый же прогон нашёл отставший размер - `gemma-4-E2B-it-Q4_K_M.gguf` за время жизни манифеста подрос на 2688 байт. Восемнадцать записей из девятнадцати оказались верны.

Что бывает и что с этим делать:

| что говорит сверка | что случилось | что делать |
|---|---|---|
| `размер` | автор пересобрал файл | `--write`, размер и сумма обновятся сами |
| `пропал` | файл переименовали или убрали | зайти на страницу репозитория и поправить `path` руками - какой файл автор имел в виду, знает только человек |
| `не вышло` | репозиторий закрыт, переехал или нет связи | если закрыт - принять лицензию на странице модели и положить токен в `HF_TOKEN` |

Уже скачанный файл после обновления размера станет считаться битым - у него прежний размер, а в манифесте новый. Программа предложит его перекачать, и это правильно: на сервере теперь лежит другой файл.

Если добывать размер всё-таки руками, то так:
...
Появилось в 1.0.6. Долго этого не было намеренно: удалять 95 ГиБ должен человек руками. Но освобождать место всё равно приходится, а руками это пятнадцать путей по разным папкам, и промахнуться там проще, чем кажется.

В окне - кнопка «Удалить выбранное», справа и поодаль от остальных. В консоли:

```bash
python install.py --remove ltx qwen
```

Так она только показывает список и объём. Чтобы удалить на самом деле:

```bash
python install.py --remove ltx qwen --yes
```

Границы жёсткие, потому что вернуть удалённое можно только заново скачав:

- **Стираются ровно те пути, что записаны в `models.json`.** Папки не трогаются вовсе - рекурсивного удаления в коде нет ни строчки. Посторонние файлы в тех же папках остаются на месте.
- **Границу стережёт та же проверка, что и на записи.** `..`, буква диска и имена устройств в `dest` отвергаются при загрузке манифеста, так что стереть мимо папки ComfyUI нечем.
- **Недокачанный `.part` уходит вместе с файлом.** Иначе вышло бы «удалил, а место не освободилось» - на тридцати гигабайтах такой хвост заметен.
- **Файл, нужный ещё и другой группе, пропускается** и называется в отчёте вместе с этими группами. Иначе снос одной группы оставил бы вторую навсегда неполной, а причину человек искал бы в скачивании. Назови обе группы - и он удалится.
- **Занятый файл не роняет остальное.** Windows не даёт удалить то, что держит открытым запущенный ComfyUI; такие файлы перечисляются отдельно, остальные уходят.
- **Во время закачки кнопка не работает**, и в консоли `--remove` требует названий групп или `--all` - снести всё случайной командой нельзя.
...
Контрольная сумма считается на лету, пока байты и так текут через скачивание, и сверяется перед тем, как файл встанет на место. Отдельным проходом по файлу это стоило бы чтения всех 95 ГиБ - оттого раньше сумм и не было, - а так почти даром. Сами суммы лежат в `models.json` в поле `sha256`, и берутся они не из воздуха: Hugging Face отдаёт их в той же описи репозитория, по которой работает `--sync-manifest`.

Не сошлась - файл на место не встаёт и остаётся в `.part`. Стереть его программа не может: отличить побитый файл от устаревшей суммы в манифесте ей нечем, а стереть вслепую значит выбросить гигабайты по догадке. Молча качать заново тоже нельзя - если врёт манифест, круг будет вечным. Поэтому она останавливается и пишет, что именно решать.

`--check` при этом остаётся по размеру: гонять хеш от 95 ГиБ на каждой проверке долго, а сумма и так сверяется в тот единственный момент, когда файл появляется. Запись без `sha256` работает как раньше и ничего не стоит.

А вот у файлов, скачанных раньше, сумму не проверял никто - их-то программа при себе не качала. Между тем порча диска это ровно тот случай, ради которого суммы и заводят, и размер её не видит: побитый сектор длину файла не меняет. Для этого есть отдельная команда:

```bash
python install.py --verify
```

Она читает файлы целиком и считает их sha256 заново. Это минут двадцать на 95 ГиБ, поэтому она и отдельная, а не внутри `--check`. Можно назвать группы, чтобы проверить только их. Про файлы без суммы в манифесте она честно говорит, что проверить их нечем.

Битый файл сам по себе не перекачается: размер у него верный, и установка сочтёт его готовым. Его надо удалить руками, и `--verify` про это пишет прямо.
...
Удаление есть, но нарочно неудобное: без `--yes` в консоли и без подтверждения в окне не пропадёт ничего. Подробности - в разделе «Удаление моделей».

## Скриншоты собираются программой

`tools/make_screenshots.py` поднимает настоящее окно, отмечает две группы, проигрывает через ту же очередь событий несколько кадров прогресса и снимает виджет через `QWidget.grab()` - по кадру на вкладку. Сеть при этом не трогается: кадры считает `core.QueueProgress` по размерам из манифеста.

Снимок области экрана не годится дважды. Окно может оказаться позади других, и в кадр попадёт чужое содержимое. А в прошлых снимках, сделанных руками, стояла папка `D:\ComfyUI` и «свободно 131.2 ГиБ» - диска D на машине нет, то есть картинка показывала то, чего программа показать не могла.
