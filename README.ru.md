...
|---|---|---|
| `InstallerModels-Setup-2.1.0.exe` | обычная установка с ярлыками | ничего |
| `InstallerModels-portable-2.1.0.zip` | распаковать и запускать, без установки | ничего |
...
Запусти `InstallerModels-Setup-2.1.0.exe`. Ставится в папку пользователя (`%LOCALAPPDATA%\Programs\InstallerModels`), права администратора не нужны, UAC не выскочит. Создаёт ярлык в меню Пуск, запись в «Программы и компоненты» для нормального удаления и, если не снять галочку, ярлык на рабочем столе. Галочка снимается на странице выбора компонентов.
...
Распакуй `InstallerModels-portable-2.1.0.zip` в любую папку (можно на флешку) и запусти `InstallerModels\InstallerModels.exe`. Ничего никуда не пишет, кроме самих моделей и запомненной папки ComfyUI. Рядом с exe лежит `models.json` - его можно править, программа возьмёт его вместо встроенного.
...
Программа переносится копированием: возьми `dist/InstallerModels-portable-2.1.0.zip` (или папку `dist/app/InstallerModels` целиком) - там exe, `_internal`, `models.json`, оба README и `docs`. Python на новой машине не нужен, он внутри. Нужны только Windows и x64. Либо просто поставь там `InstallerModels-Setup`.
