...
Запусти `InstallerModels-Setup-1.0.0.exe`. Ставится в папку пользователя (`%LOCALAPPDATA%\Programs\InstallerModels`), права администратора не нужны, UAC не выскочит. Создаёт ярлык в меню Пуск, запись в «Программы и компоненты» для нормального удаления и, если не снять галочку, ярлык на рабочем столе. Галочка снимается на странице выбора компонентов.
...
Первый раз - по заголовку ответа, до того как на диск попадёт хоть один байт. При обычном запросе это `Content-Length`, при докачке - число после дробной черты в `Content-Range`. Если сервер называет не тот размер, значит `models.json` устарел, и качать нечего: программа сразу пишет, сколько на сервере и сколько записано у неё. Раньше расхождение вылезало только в самом конце, и цикл повторов успевал скачать файл трижды. На файле в 27 ГиБ это около 81 ГиБ трафика ради одного неверного числа.
