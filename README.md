...
|---|---|---|
| `InstallerModels-Setup-1.0.6.exe` | обычная установка с ярлыками | ничего |
...
Запусти `InstallerModels-Setup-1.0.6.exe`. Ставится в папку пользователя (`%LOCALAPPDATA%\Programs\InstallerModels`), права администратора не нужны, UAC не выскочит. Создаёт ярлык в меню Пуск, запись в «Программы и компоненты» для нормального удаления и, если не снять галочку, ярлык на рабочем столе. Галочка снимается на странице выбора компонентов.
...
Программа переносится копированием: возьми папку `dist/portable` целиком - там exe, `models.json`, `README.md` и `docs`. Python на новой машине не нужен, он внутри exe. Нужны только Windows и x64. Либо просто поставь там `InstallerModels-Setup`.
...
Всего 59 проверок, за четырьмя из них 2640 порождённых, весь прогон - пять секунд. Проверками накрыты и консоль (коды возврата, разбор команды, установка от начала до конца), и окно (сборка виджетов, насос событий, три исхода закачки), и сама сборка, и `setup.nsi`.
