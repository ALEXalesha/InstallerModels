...
Установщик и деинсталлятор отказываются работать, пока окно программы открыто. Ищут они его по заголовку окна, так что если переименовать окно в `gui.py` и забыть про `WINTITLE` в `setup.nsi`, проверка тихо перестанет срабатывать.
