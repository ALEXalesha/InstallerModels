...
**Возвращает модели ComfyUI обратно на диск: качает с Hugging Face напрямую, с докачкой после обрыва, проверкой размера и sha256. 15 файлов на 95.4 ГиБ, разложенных по группам под конкретные воркфлоу.**
...
|---|---|---|
| `InstallerModels-Setup-2.0.0.exe` | обычная установка с ярлыками | ничего |
| `InstallerModels-portable-2.0.0.zip` | распаковать и запускать, без установки | ничего |
...
Запусти `InstallerModels-Setup-2.0.0.exe`. Ставится в папку пользователя (`%LOCALAPPDATA%\Programs\InstallerModels`), права администратора не нужны, UAC не выскочит. Создаёт ярлык в меню Пуск, запись в «Программы и компоненты» для нормального удаления и, если не снять галочку, ярлык на рабочем столе. Галочка снимается на странице выбора компонентов.
...
Распакуй `InstallerModels-portable-2.0.0.zip` в любую папку (можно на флешку) и запусти `InstallerModels\InstallerModels.exe`. Ничего никуда не пишет, кроме самих моделей и запомненной папки ComfyUI. Рядом с exe лежит `models.json` - его можно править, программа возьмёт его вместо встроенного.
...
Вкладка LM Studio - названия моделей с кнопкой «Копировать». Рядом с квантом показан ключ модели в LM Studio - тот, которым её зовут из `lms load` и из API. Он лежал в `models.json` и был расписан в документации, а сама программа его не показывала нигде. Скопированное переживает закрытие окна: Qt при выходе сам отдаёт буфер обмена системе. (На tkinter до версии 1.0.5 «скопировал и закрыл» означало «вставлять нечего» - ровно тот порядок действий, на который кнопка и рассчитана.)
...
Программа переносится копированием: возьми `dist/InstallerModels-portable-2.0.0.zip` (или папку `dist/app/InstallerModels` целиком) - там exe, `_internal`, `models.json`, оба README и `docs`. Python на новой машине не нужен, он внутри. Нужны только Windows и x64. Либо просто поставь там `InstallerModels-Setup`.
...
| `requirements.txt`, `requirements-dev.txt` | PySide6 для окна; PyInstaller для сборки |
